...
import draggable, { dropzone } from "../lib/draggable.ts";
...
function endEditingAlias(save = true) {
    ...
    if (save && editingAliasEl?.value?.length && editingAliasEl.value !== alias)
        editingAliasEl.value = setAlias(editingAliasEl?.value ?? "sequence");
}
</script>
...
.sequence {
    ...
    transition: margin-block 200ms ease;
}
